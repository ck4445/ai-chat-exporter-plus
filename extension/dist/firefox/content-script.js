...
(function () {
  ...
  const Utils = {
    ...
    /**
     * Truncates a string to a given maximum length, adding "â€¦" if truncated.
     * @param {string} str The input string.
     * @param {number} [len=70] The maximum length.
     * @returns {string} The truncated string.
     */
    truncate(str, len = 70) {
      return str.length <= len ? str : str.slice(0, len).trim() + "â€¦";
    },
  ...
  const UIManager = {
    ...
    addExportControls() {
      ...
      markdownButton.textContent = "â¬‡ Export MD";
      ...
      settingsButton.title = `${EXPORT_BUTTON_TITLE_PREFIX}: âš™ï¸ Settings: Configure Filename Format`;
      ...
      settingsButton.addEventListener("click", () => {
        ...
        const newFormat = window.prompt(
          `+++++++  ${EXPORT_BUTTON_TITLE_PREFIX}  +++++++\n\n ` +
            ...
            ` â€¢ sample1: {platform}__{tag1}__{title}__{timestampLocal}\n` +
            ` â€¢ sample2: {tag1}__{title}-v{exporter}-{timestamp}\n` +
            ` â€¢ current: ${currentFormat}\n\n` +
            `valid placeholders: \n  ` +
    ...
    generateOutlineContent() {
      ...
      toggleButton.textContent = UIManager._outlineIsCollapsed ? "â–²" : "â–¼"; // Up/Down arrow
